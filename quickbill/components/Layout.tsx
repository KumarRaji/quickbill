...
const Layout: React.FC<LayoutProps> = ({ currentView, onChangeView, user, onLogout, children }) => {
  ...
  // Expanded permissions: Super Admin can do everything Admin can
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 text-white shadow-xl">
        <div className="p-6 flex items-center space-x-3 border-b border-slate-700">
          <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center font-bold text-xl">Q</div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          />
          <NavItem 
            active={currentView === 'SALES_RETURNS'} 
            onClick={() => onChangeView('SALES_RETURNS')} 
            icon={Undo2} 
            label="Sale Return" 
          />
        ...
        <div className="p-4 border-t border-slate-700">
           <div className="flex items-center space-x-3 mb-4 px-2">
              ...
              <div className="overflow-hidden">
                ...
                <div className="text-xs text-slate-400 capitalize">{user.role.toLowerCase().replace('_', ' ')}</div>
              </div>
           </div>
          <button 
            onClick={onLogout}
      ...
      <div className="flex-1 flex flex-col h-full w-full relative">
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-16 left-0 w-full h-[calc(100vh-4rem)] bg-slate-900 text-white z-30 p-4 md:hidden overflow-y-auto">
             ...
             <button 
                onClick={() => { onChangeView('CREATE_TRANSACTION'); setIsMobileMenuOpen(false); }}
            ...
            <nav className="space-y-2">
              <NavItem active={currentView === 'DASHBOARD'} onClick={() => { onChangeView('DASHBOARD'); setIsMobileMenuOpen(false); }} icon={LayoutDashboard} label="Dashboard" />
              <NavItem active={currentView === 'PARTIES'} onClick={() => { onChangeView('PARTIES'); setIsMobileMenuOpen(false); }} icon={Users} label="Parties" />
              <NavItem active={currentView === 'ITEMS'} onClick={() => { onChangeView('ITEMS'); setIsMobileMenuOpen(false); }} icon={Package} label="Items" />
              <NavItem active={currentView === 'STOCK'} onClick={() => { onChangeView('STOCK'); setIsMobileMenuOpen(false); }} icon={Boxes} label="Stock" />
              {canManageData && <NavItem active={currentView === 'REPORTS'} onClick={() => { onChangeView('REPORTS'); setIsMobileMenuOpen(false); }} icon={BarChart3} label="Reports" />}
              
              <div className="border-t border-slate-800 my-2 pt-2 text-xs text-slate-500 uppercase">Sales</div>
              <NavItem active={currentView === 'SALES_INVOICES'} onClick={() => { onChangeView('SALES_INVOICES'); setIsMobileMenuOpen(false); }} icon={FileText} label="Sale Invoices" />
              <NavItem active={currentView === 'PAYMENT_IN'} onClick={() => { onChangeView('PAYMENT_IN'); setIsMobileMenuOpen(false); }} icon={TrendingDown} label="Payment In" />
              
              <div className="border-t border-slate-800 my-2 pt-2 text-xs text-slate-500 uppercase">Purchases</div>
              <NavItem active={currentView === 'PURCHASE_INVOICES'} onClick={() => { onChangeView('PURCHASE_INVOICES'); setIsMobileMenuOpen(false); }} icon={ShoppingCart} label="Purchase Bills" />
              <NavItem active={currentView === 'PAYMENT_OUT'} onClick={() => { onChangeView('PAYMENT_OUT'); setIsMobileMenuOpen(false); }} icon={TrendingUp} label="Payment Out" />
              {canManageData && <NavItem active={currentView === 'EXPENSES'} onClick={() => { onChangeView('EXPENSES'); setIsMobileMenuOpen(false); }} icon={Wallet} label="Expenses" />}
              
              {isSuperAdmin && (
                 <>
                    <div className="border-t border-slate-800 my-2 pt-2 text-xs text-slate-500 uppercase">Admin</div>
                    <NavItem active={currentView === 'USERS'} onClick={() => { onChangeView('USERS'); setIsMobileMenuOpen(false); }} icon={ShieldCheck} label="Manage Users" />
                 </>
              )}
            </nav>
          </div>
