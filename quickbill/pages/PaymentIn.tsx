...
const PaymentIn: React.FC<PaymentInProps> = ({ parties, onRefresh }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchPayments();
      onRefresh(); // Refresh parent data (party balances)
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">Party</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">Mode</th>
                <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 text-right">Amount Received</th>
              </tr>
            </thead>
