...
const InvoiceView: React.FC<InvoiceViewProps> = ({ invoice, onBack, autoPrint = false }) => {
  ...
  useEffect(() => {
    if (autoPrint) {
      // Default to A4 for auto-print, or could be configurable
  ...
  const switchToThermalAndPrint = () => {
    setViewMode('THERMAL');
    // Wait for render to update before printing
    setTimeout(() => {
      window.print();
    }, 100);
  };

  const switchToA4AndPrint = () => {
    setViewMode('A4');
    setTimeout(() => {
      window.print();
    }, 100);
  ...
  const getDocTitle = () => {
    switch (invoice.type) {
      ...
      case 'RETURN': return 'CREDIT NOTE';
    ...
    <div className="bg-white max-w-4xl mx-auto shadow-xl p-8 min-h-[1100px] print:shadow-none print:min-h-0 print:p-0">
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-100 pb-8 mb-8">
        ...
        <div className="text-right">
          <h2 className="text-2xl font-bold mb-2 uppercase tracking-widest text-slate-300">{getDocTitle()}</h2>
          <div className="text-slate-600">
            <span className="font-bold"># {invoice.invoiceNumber}</span>
            <br/>
            Date: {new Date(invoice.date).toLocaleDateString()}
            {invoice.originalRefNumber && (
              <>
                <br/>
                <span className="text-sm">Ref No: {invoice.originalRefNumber}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="mb-8 p-6 bg-slate-50 rounded-lg border border-slate-100">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-1">{invoice.partyName}</h2>
        ...
        <tbody className="divide-y divide-slate-100">
          {invoice.items.map((item, idx) => (
            <tr key={idx}>
              ...
              <td className="px-4 py-3 text-right text-slate-800 font-bold">₹{item.amount.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="border-t border-slate-100 pt-8">
          <h4 className="text-sm font-bold text-slate-800 mb-2">Terms & Conditions</h4>
  ...
  const ThermalLayout = () => (
    <div className="bg-white mx-auto p-2 shadow-xl print:shadow-none w-[300px] print:w-full font-mono text-sm text-black leading-tight">
      ...
      {invoice.originalRefNumber && (
        <div className="flex justify-between text-xs mb-1">
          <span>Ref:</span>
          <span>{invoice.originalRefNumber}</span>
        </div>
      )}
      ...
      <div className="mb-2">
        ...
        <span className="text-xs">{invoice.partyName}</span>
      </div>
        ...
        {invoice.items.map((item, idx) => (
          <div key={idx} className="mb-1">
            ...
            <div className="flex text-xs text-slate-600">
              ...
              <div className="w-1/4 text-center">{item.quantity} x {item.price}</div>
              <div className="w-1/4 text-right text-black">{item.amount.toFixed(2)}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex space-x-3 bg-white p-1 rounded-lg shadow-sm border border-slate-200">
           <button 
            ...
            className={`flex items-center space-x-2 px-3 py-2 rounded-md transition-colors ${
              viewMode === 'A4' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            <FileText size={18} />
          ...
          <button 
            ...
            className={`flex items-center space-x-2 px-3 py-2 rounded-md transition-colors ${
              viewMode === 'THERMAL' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            <Receipt size={18} />
